        ...
        >
          ...
          <header className="sticky top-0 z-10 flex h-14 items-center justify-between border-b border-white/[0.06] bg-[#050816]/90 px-5 backdrop-blur-sm">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <div className="text-xs font-semibold tracking-wider" style={{ color: "#d4a032", fontFamily: "Chakra Petch" }}>SETTINGS</div>
                <div className="text-[9px] uppercase tracking-widest" style={{ color: "#6b6b7b", fontFamily: "JetBrains Mono" }}>Connection & Config</div>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <main className="mx-auto max-w-[800px] px-5 py-10">
            ...
            <section className="mb-8">
              <h3 className="mb-4 text-[10px] font-bold uppercase tracking-[0.2em]" style={{ color: "#6b6b7b", fontFamily: "JetBrains Mono" }}>Connection</h3>
              <div className="rounded-lg border border-white/[0.04] bg-white/[0.01] p-5">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <span className="grid h-10 w-10 place-items-center rounded-lg border" style={{
                      borderColor: source === "tuya" ? "rgba(45,212,191,0.2)" : source === "offline" ? "rgba(220,68,70,0.2)" : "rgba(212,160,50,0.2)",
                      background: source === "tuya" ? "rgba(45,212,191,0.05)" : source === "offline" ? "rgba(220,68,70,0.05)" : "rgba(212,160,50,0.05)",
                    }}>
                      {source === "tuya" ? <Wifi className="h-5 w-5 text-[#2dd4bf]" /> :
                       source === "offline" ? <WifiOff className="h-5 w-5 text-[#dc4446]" /> :
                       <RefreshCw className="h-5 w-5 text-[#d4a032]" />}
                    </span>
                    <div>
                      <div className="text-sm font-semibold" style={{ color: "#e2e2e8", fontFamily: "Chakra Petch" }}>
                        {source === "tuya" ? "Tuya IoT — LIVE" :
                         source === "offline" ? "Tuya IoT — OFFLINE" :
                         source === "connecting" ? "Connecting..." :
                         "Simulation Mode"}
                      </div>
                      <div className="text-[10px]" style={{ color: "#6b6b7b", fontFamily: "JetBrains Mono" }}>
                  ...
                  <span className="flex items-center gap-1.5 text-[10px] font-semibold" style={{
                    color: source === "tuya" ? "#2dd4bf" : source === "offline" ? "#dc4446" : "#d4a032",
                    fontFamily: "JetBrains Mono",
                  }}>
                    {source === "tuya" ? <CheckCircle className="h-3 w-3" /> :
                     source === "offline" ? <XCircle className="h-3 w-3" /> :
                     <RefreshCw className={`h-3 w-3 ${source === "connecting" ? "animate-spin" : ""}`} />}
                    {source === "tuya" ? "CONNECTED" :
                     source === "offline" ? "DISCONNECTED" :
                     source === "connecting" ? "PENDING" :
                     "SIMULATION"}
                  </span>
                </div>
            ...
            {/* Configuration */}
            <section>
              <h3 className="mb-4 text-[10px] font-bold uppercase tracking-[0.2em]" style={{ color: "#6b6b7b", fontFamily: "JetBrains Mono" }}>Environment</h3>
              <div className="rounded-lg border border-white/[0.04] bg-white/[0.01] p-5">
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between rounded-md border border-white/[0.04] px-3 py-2.5">
                    <span className="text-[10px] font-semibold uppercase tracking-wider" style={{ color: "#6b6b7b", fontFamily: "JetBrains Mono" }}>ENDPOINT</span>
                    <span className="text-[10px] font-mono" style={{ color: "#e2e2e8" }}>{config.endpoint}</span>
                  </div>
                  {deviceId && (
                    <div className="flex items-center justify-between rounded-md border border-white/[0.04] px-3 py-2.5">
                      <span className="text-[10px] font-semibold uppercase tracking-wider" style={{ color: "#6b6b7b", fontFamily: "JetBrains Mono" }}>ACTIVE DEVICE</span>
                      <span className="text-[10px] font-mono" style={{ color: "#e2e2e8" }}>DEV-{deviceId.substring(0, 8)}</span>
                    </div>
                  )}
                </div>
                ...
                {!allConfigured && (
                  <div className="mt-4 rounded-md border border-[#d4a032]/20 bg-[#d4a032]/5 px-4 py-3">
                    <p className="text-[10px] leading-relaxed" style={{ color: "#d4a032", fontFamily: "JetBrains Mono" }}>
                      ⚡ Tuya credentials not fully configured. Set <span className="font-bold">VITE_TUYA_CLIENT_ID</span>, <span className="font-bold">VITE_TUYA_CLIENT_SECRET</span>, and <span className="font-bold">VITE_TUYA_DEVICE_ID</span> in <span className="font-bold">.env</span> to connect live.
                    </p>
                  </div>
                )}
              </div>
            ...
            <section className="mt-8">
              <h3 className="mb-4 text-[10px] font-bold uppercase tracking-[0.2em]" style={{ color: "#6b6b7b", fontFamily: "JetBrains Mono" }}>System</h3>
              <div className="rounded-lg border border-white/[0.04] bg-white/[0.01] p-5">
                <div className="flex items-center gap-3">
                  <Cpu className="h-5 w-5" style={{ color: "#3a3a4a" }} />
                  <div className="text-[10px]" style={{ color: "#6b6b7b", fontFamily: "JetBrains Mono" }}>
                    Solar Monitor — Radiant Command Core<br />
...
function ConfigRow({ label, ok }: { label: string; ok: boolean }) {
  return (
    <div className="flex items-center justify-between rounded-md border border-white/[0.04] bg-white/[0.01] px-3 py-2.5">
      <span className="text-[10px] font-semibold uppercase tracking-wider" style={{ color: "#6b6b7b", fontFamily: "JetBrains Mono" }}>{label}</span>
      <span className="flex items-center gap-1.5 text-[10px] font-semibold" style={{
